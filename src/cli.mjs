...
async function uploadUnsigned({ cloudName, uploadPreset, folder, filePath, publicId, signal }) {
  ...
  return { ok, status: resp.status, durationMs: Math.round(t1 - t0), data };
}
  ...
  for (let b = 0; b < batches; b++) {
    ...
    if (dry) {
      ...
      const promises = files.map(async (f) => {
        ...
        let result;
        try {
          result = await uploadUnsigned({ cloudName, uploadPreset, folder, filePath: f.path, publicId });
          await writeLogLine(logPath, {
            ts: new Date().toISOString(),
            runId, batch: batchNo, file: f.name, path: f.path, size: f.size,
